...
from model import Mail, Activity

class MainPage(webapp.RequestHandler):
    def get(self):
        getActivityId = int(str(self.request.url).split('/Sendmail/id_')[1])
        searchActivityInfos = db.GqlQuery("SELECT * FROM Activitydb WHERE activity_id = :1",getActivityId)
        searchActivityInfo = searchActivityInfos.get()
        
        searchMailInfos = db.GqlQuery("SELECT * FROM Maildb")  #get activity info.
